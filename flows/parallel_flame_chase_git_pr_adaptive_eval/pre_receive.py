...
def protect_eval_gate(repository: Path, old: str, new: str) -> None:
    """Allow only linear coordinator gate commits on the dedicated control ref."""
    if set(new) == {"0"}:
        reject("the adaptive evaluation gate ref cannot be deleted")
    fields = run_git(repository, "rev-list", "--parents", "-n", "1", new).split()
    if len(fields) != 2:
        reject("an adaptive evaluation gate update must have exactly one parent")
    parent = fields[1]
    if set(old) != {"0"} and parent != old:
        reject("adaptive evaluation gate updates must be linear")
    paths = [
        path
        for path in run_git(repository, "diff", "--name-only", "-z", parent, new).split(
            "\0"
        )
        if path
    ]
    if not paths or any(not path.startswith(".pfc/adaptive-eval/") for path in paths):
        reject("gate commits may change only .pfc/adaptive-eval/**")
    if ".pfc/adaptive-eval/manifest.json" not in paths:
        reject("each gate commit must update .pfc/adaptive-eval/manifest.json")
...
def main() -> None:
    ...
    for line in sys.stdin:
        ...
        elif reference == "refs/heads/pfc/eval-gate":
            protect_eval_gate(repository, old, new)
